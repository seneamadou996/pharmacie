...
<?php 
    ...
    $ps =$req->prepare("SELECT * FROM stocks");
    $ps->execute();
    $ps1=$req->prepare("SELECT * FROM medicaments");
    $ps1->execute();
    $ps2=$req->prepare("SELECT * FROM commandes");
    $ps2->execute();
    $time=date('Y-m-d');
    if(isset($_POST['ok'])){
        $medicament=$_POST['medicaments_id'];
        $stock=$_POST['stocks_id'];
        $commande=$_POST['commandes_id'];
        $quantite=$_POST['quantite'];
        $date_fab=$_POST['date_fab'];
        $date_peremp=$_POST['date_peremp'];
        $date_stock=$_POST['date_stock'];
        $ps3=$req->prepare("SELECT MAX(quantite_stock) AS quante  FROM stockagemedicaments WHERE medicaments_id=$medicament");
        $ps3->execute();
        $quant=$ps3->fetch();
        $quant_stock=$quant['quante'];
        $qte_stock=$quant_stock;
        $qte_stock +=$quantite;
        $ps4=$req->prepare("SELECT SUM(quantite_sotck) AS total FROM stockagemedicaments WHERE id=$stock");
        $ps4->execute();
        $res=$ps4->fetch();
        $result=$res['total'];
        $qte_tot=$result+$quantite;
        $qte_ref=$ps->fetch();
        if($qte_tot>$qte_ref['quantite_max']){
            echo "<strong style='color:red'>vous avez atteint la quantité maximale du stock</strong>";
        }else{
            $int=$req->prepare("INSERT INTO stockagemedicaments(quantite,quantite_stock,date_fab,date_peremp,date_stock,stocks_id,medicaments_id,commandes_id)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?)");
            $params=array($quantite,$qte_stock,$date_fab,$date_peremp,$date_stock,$stock,$medicament,$commande);
            $int->execute($params);
            echo "<strong style='color:green'>stockage réussie avec succés</strong>";
        ...
        <div class="card">
            ...
            <div class="card-body bg-dark col-md-12 col-xs-12">
                <div class="row cont collapsed">
                            ...
                            <li class="nav-item">
                                ...
                                <div id="collapseUtilities" class="collapse" aria-labelledby="headingUtilities" data-parent="#accordionSidebar">
                                    <div class="bg-white py-2 collapse-inner rounded">
                                        ...
                                        <a class="collapse-item" href="index.php">Stockage médicament</a>
                    ...
                    <div class="col-9">
                        <h2 class="page-header text-white mt-5">Gestion de stock</h2>
                        <hr class="bg-white">
                        <div class="container mt-5">
                        <form id="frm-alumno formulaire" name="formulaire" action="index.php" method="post" enctype="multipart/form-data">
                            <div class="form-row">
                                <div class="col">
                                    <select name="stocks_id" id="stocks_id" class="form-group ml-2 mt-2">
                                        <option class="form-control" value="0" selected disabled>Selectionnez un stock</option>
                                        <?php while($i=$ps->fetch()) { ?>
                                            <option class="form-control" value="<?php echo($i['id'])?>"><?php echo($i['libelle_stock'])?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="col">
                                    <select name="medicaments_id" id="medicaments_id" class="form-group ml-2 mt-2">
                                        <option class="form-control" value="0" selected disabled>Selectionnez un médicament</option>
                            ...
                            <div class="form-row">
                                <div class="col">
                                    <select name="commandes_id" id="commandes_id" class="form-group ml-2 mt-2">
                                        <option class="form-control" value="0" selected disabled>Selectionnez une commande</option>
                                        <?php while($i=$ps2->fetch()) { ?>
                                            <option class="form-control" value="<?php echo($i['id'])?>"><?php echo($i['numero_com'])?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="col">
                            ...
                            <div class="row mb-3">
                                <div class="col">
                                    <input type="date" class="form-control" name="date_fab" placeholder="Date de fabrication" required>
                                </div>
                                <div class="col">
                                    <input type="date" class="form-control" name="date_peremp" placeholder="Date de péremption" required>
                                </div>
                            </div>
                            <div class="form-row mb-3">
                                <div class="col">
                                    <input type="date" class="form-control" value="<?php echo $time; ?>" name="date_stock" placeholder="Date de stockage" required>
                                </div>
                                <div class="col mt-1">
